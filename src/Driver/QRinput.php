...
/**
 * Aggregates input segments and builds the final QR bit/byte streams.
 *
 * Validates segments for the chosen version/EC level, estimates version when
 * not specified, and converts data into interleavable codewords.
 */
class QRinput {
    ...
    public function __construct(int $version = 0, int $level = qrstr::ECLEVEL_L) {
        if ($version < 0 || $version > QRspec::VERSION_MAX || $level > qrstr::ECLEVEL_H) {
    ...
    public function setVersion(int $version): int {
        if ($version < 0 || $version > QRspec::VERSION_MAX) {
            throw new Exception('Invalid version no');
        }
    ...
    public function insertStructuredAppendHeader($size, $index, $parity): int {
        ...
        try {
            $entry = new QRinputItem(qrstr::MODE_STRUCTURE, 3, $buf);
    ...
    public function calcParity(): int {
        ...
        foreach ($this->items as $item) {
            if ($item->mode != qrstr::MODE_STRUCTURE) {
    ...
    public static function estimateBitsModeNum($size): mixed {
        $w = (int)$size / 3;
        $bits = $w * 10;
    ...
    public static function lookAnTable($c): mixed {
    ...
    public static function estimateBitsMode8($size): int|float {
        return $size * 8;
    }
    ...
    public function estimateBitStreamSize($version): mixed {
        $bits = 0;
    ...
    public function appendPaddingBit(&$bstream): mixed {
        $bits = $bstream->size();
